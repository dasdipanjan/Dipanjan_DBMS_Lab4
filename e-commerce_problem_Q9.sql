USE `ecommercedb`;
DROP procedure IF EXISTS `displaySupplierInformation`;

USE `ecommercedb`;
DROP procedure IF EXISTS `ecommercedb`.`displaySupplierInformation`;
;

DELIMITER $$
USE `ecommercedb`$$
CREATE PROCEDURE `displaySupplierInformation` ()
BEGIN
SELECT 
    report.supp_id,
    report.supp_name,
...
CALL displaySupplierInformation;